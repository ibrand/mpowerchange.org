			...
			<div class="small-12 small-centered medium-6 large-4 xlarge-3 columns">
					...
					<div class="login-container">
						...
						<form method="post" class="woocommerce-form woocommerce-form-login login text-center">
								...
								<input type="text" class="input-text full" name="username" id="username" placeholder="<?php _e( 'Username or email address', 'thevoux' ); ?>"/>

								<input class="input-text full" type="password" name="password" id="password" placeholder="<?php _e( 'Password', 'thevoux' ); ?>"/>
							<div class="row">
					...
					<div class="register-container">
						...
						<form method="post" class="woocommerce-form woocommerce-form-register register text-center">
							<?php do_action( 'woocommerce_register_form_start' ); ?>
							<?php if (get_option('woocommerce_registration_generate_username')=='no') : ?>
									<input type="text" class="input-text full" name="username" id="reg_username" value="<?php if (isset($_POST['username'])) echo esc_attr($_POST['username']); ?>" placeholder="<?php _e( 'Username', 'thevoux' ); ?>" />
						
							<?php else : endif; ?>
								<input type="email" class="input-text full" name="email" id="reg_email" value="<?php if (isset($_POST['email'])) echo esc_attr($_POST['email']); ?>" placeholder="<?php _e( 'Email address', 'thevoux' ); ?>" />
							<?php if (get_option('woocommerce_registration_generate_password')=='no') : ?>
								<input type="password" class="input-text full" name="password" id="reg_password" value="<?php if (isset($_POST['password'])) echo esc_attr($_POST['password']); ?>" placeholder="<?php _e( 'Password', 'thevoux' ); ?>" />
							<?php endif; ?>
