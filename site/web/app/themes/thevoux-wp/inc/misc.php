...
function thb_body_classes( $classes ) {
	...
	$header_submenu_color = 'header_submenu_color-'.ot_get_option('header_submenu_color', 'light');
	$mobile_menu_animation = 'mobile_menu_animation-'.ot_get_option('mobile_menu_animation');
	$site_borders = 'thb-borders-'.ot_get_option('site_borders', 'off');
	$header_submenu_style = 'header-submenu-'.ot_get_option('header_submenu_style', 'style1');
	$rounded_forms = 'thb-rounded-forms-'.ot_get_option('rounded_forms', 'off');
	$classes[] = $site_borders;
	$classes[] = $rounded_forms;
	$classes[] = $social_black;
	$classes[] = $header_submenu_color;
	$classes[] = $mobile_menu_animation;
	$classes[] = $header_submenu_style;
	return $classes;
}
...
function thb_my_new_contactmethods( $contactmethods ) {
	...
	// Add Google+
	$contactmethods['googleplus'] = esc_html__('Google Plus URL', 'thevoux');
	// Add Instagram
...
/* Thb Mobile Toggle */
function thb_mobile_toggle($small = false) {
	$mobile_icon = ot_get_option('mobile_menu_icon', 'on');
	
	$classes[] = 'mobile-toggle';
	$classes[] = $mobile_icon !== 'on' ? 'hide-for-large' : false;
...
function thb_secondary_area($menu = false) {
	if ($menu) {
		...
		<?php
		  wp_nav_menu( array( 'theme_location' => 'secondary-menu', 'depth' => 2, 'container' => false, 'menu_class' => 'secondary-menu full-menu nav submenu-style-' . ot_get_option('header_submenu_style', 'style1'), 'walker' => new thb_MegaMenu_tagandcat_Walker ) );
		?>
		</nav>
		<?php
		}
	...
	<div class="thb-post-top">
		...
		<?php if ( $time ) { ?>
			<aside class="post-date">
				<?php echo thb_human_time_diff_enhanced(); ?>
			</aside>
		<?php } ?>
	</div>
	<?php
...
function thb_footer_items() {
	...
	?>
		<a href="#" title="<?php esc_attr_e('Scroll To Top', 'revolution'); ?>" id="scroll_totop">
